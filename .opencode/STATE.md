...
| 3 | `benchmarks/results/phase-3.json` | 1.0 | result | 10 Terminal-Bench tasks; Harbor fallback mode (no Harbor/credentials in env); gate PASS vs phase-2 baseline; real Harbor eval deferred until credentials exist (backfilled post-Phase-7 audit) |
...
| 5 | `benchmarks/results/phase-5.json` | 1.0 | mock→mock | Internal OQS evaluation (Phase 4 baseline); self-improvement loop components; gate PASS vs phase-4 baseline (backfilled post-Phase-7 audit) |
...
- 2026-09-19 — Phase 7 ledger follow-up merged via PR #10 (squash eff1376): STATE.md records PR #9 merge, benchmark baseline row for phase 7. develop == eff1376.
- 2026-09-19 — fix-known-gaps audit on `chore/fix-known-gaps` (work in progress, not yet merged): npm scope renamed `@forge/*` → `@runforge/*` (all 4 packages + docs + imports + workflows; registry re-verified: `@runforge/{cli,core,contracts,adapters}` unclaimed, `@forge/cli` taken by Atlassian); LICENSE added (Apache 2.0, copyright 2026 cpratham16) + `license: Apache-2.0` in all 5 package.json; ADRs renumbered 002→011 (fail-closed-policy) and 003→012 (no-false-parity), PROJECT_OVERVIEW §9 table now the true registry (ADR-002 trace-first, ADR-003 evidence-based, new 011/012 rows), AGENTS.md §2 ADR-numbering rule added; CODEOWNERS `@OWNER` → `@cpratham16` + bootstrap.sh message fixed (root `CODEOWNERS`, both copies); README claims test added (`scripts/check-readme-claims.mjs`, `pnpm check-claims`, wired into ci.yml + called out in phase-merge.md; README status line fixed to Phase 7); benchmark baselines phase-3.json and phase-5.json backfilled (were missing → Phase 4's gate passed via the no-baseline fallback); benchmark-gate.yml fallback now only applies to phase 2 (baseline ≤ 1), missing baseline for phase ≥ 3 fails loudly with backfill instructions; `.github/workflows/opencode.yml` restored (official `anomalyco/opencode/github@latest` wiring, verified against opencode docs).
